...
function Collection(props) {
  ...
  const changeClothes = (product, newLink) => {
    ...
    if (newLink === "") {
      ...
      if (
        product.linkProduct.includes(newLink) ||
        product.linkProductColor.includes(newLink)
      ) {
  ...
  const miniColorRight = (product) => {
    ...
    if (product.linkProductColor.length === 0 || product.color.length === 0) {
      ...
      xhtml = (
        <li
          onMouseOver={() => setIsColor(true)}
          onMouseLeave={() => setIsColor(false)}
          className=" relative flex items-center justify-center mb-2 rounded-full bg-gray-100 shadow-xl w-9 flex-wrap transform transition duration-500 ease-in-out hover:bg-gray-200"
        >
      ...
      <div className="flex flex-col text-white leading-none text-center uppercase text-sm">
        <span
          className={classNames(
            status !== "" || status === null ? "block" : "hidden",
            "bg-pink-500 py-1 px-1  mb-2 "
          )}
        >
          {status}
        </span>
        <span
          className={classNames(
            discount !== "" || discount === null ? "block" : "hidden",
            "flex flex-col bg-green-400 py-1 px-1 mb-2 "
          )}
        >
          -{discount}%<span className="">Sale</span>
        </span>
      </div>
    );
    return xhtml;
  ...
  const showCollections = (productsList, isColor, isAddToCart) => {
    ...
    if (productsList.length > 0) {
      xhtml = productsList.map((product, i) => {
        ...
        return (
          <div key={product.id} className={classNames(i >= 3 ? "md:col-start-2  lg:col-span-1" :"","w-full col-span-1 h-full group mt-8")}>
